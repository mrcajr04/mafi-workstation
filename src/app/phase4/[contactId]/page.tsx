...
import { MobileCollapsibleSummary } from "@/app/phase4/[contactId]/mobile-collapsible-summary";
  ...
  const prospectDetails = (
    <>
      <SummaryCard title="Contact">
        <SummaryItem label="Prospect" value={contact.prospectName} />
      ...
      <SummaryCard title="Final Scenario">
        ...
        <SummaryItem
          ...
          value={
            selectedScenario
              ? formatInterestRateDisplay(selectedScenario.interestRate)
              : EMPTY_VALUE
          }
        />
        <SummaryItem
          ...
          value={formatCurrencyDisplayWithCents(
            selectedScenario?.principalAndInterest,
          )}
        />
        <SummaryItem
          label="Monthly Insurance"
          value={formatCurrencyDisplay(selectedScenario?.monthlyInsurance)}
        />
        <SummaryItem
          label="PITIA"
          value={formatCurrencyDisplayWithCents(selectedScenario?.pitia)}
        />
        <SummaryItem
          label="Escrowed"
          value={
            selectedScenario
              ? selectedScenario.escrowed
                ? "Yes"
                : "No"
              : EMPTY_VALUE
          }
    ...
    <div className="max-w-[1440px] space-y-6">
      <div className="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)] xl:grid-cols-[400px_minmax(0,1fr)]">
        <aside className="hidden lg:block">
          <div className="sticky top-20 grid max-h-[calc(100vh-6rem)] gap-4 overflow-y-auto pr-1">
            {prospectDetails}
          </div>
        </aside>

        <div className="space-y-6">
          <div className="grid gap-4 lg:hidden">{prospectDetails}</div>
          ...
          <Phase4Form
            ...
            initialData={
              contact.phase4Pipeline
                ? {
                    allInvoicesCollected:
                      contact.phase4Pipeline.allInvoicesCollected,
                    appraisalStatus: contact.phase4Pipeline.appraisalStatus,
                    closingDocsSignedOut:
                      contact.phase4Pipeline.closingDocsSignedOut,
                    ...
                    fundingDate: formatDateInput(
                      contact.phase4Pipeline.fundingDate,
                    ),
                    loanApplicationStatus:
                      contact.phase4Pipeline.loanApplicationStatus,
                    loanApprovalStatus:
                      contact.phase4Pipeline.loanApprovalStatus,
                    loanLockConfirmed: contact.phase4Pipeline.loanLockConfirmed,
                    postClosingComplete:
                      contact.phase4Pipeline.postClosingComplete,
  ...
  return (
    <Card className="border-mafi-border bg-mafi-bg-white">
      <div className="md:hidden">
        <MobileCollapsibleSummary title={title}>
          {children}
